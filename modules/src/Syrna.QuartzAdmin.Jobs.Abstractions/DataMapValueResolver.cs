...
using Syrna.QuartzAdmin.Jobs.Abstractions.Processors;
        ...
        public string Resolve(DataMapValue dmv)
        {
            ...
            switch (dmv.Type)
            {
                case DataMapValueType.InterpolatedString:
                    ...
                    {
                        case 1:
                            var logger = _svcProvider?.GetRequiredService<ILogger<InterpolatedStringV1Processor>>();
                            var processor = new InterpolatedStringV1Processor(logger);
                            return processor.Process(dmv);
                        default:
                            throw new NotSupportedException($"DataMapValue {dmv.Type} version {dmv.Version} is not supported.");
                    }
                default:
                    throw new NotSupportedException($"DataMapValueType {dmv.Type} is not supported.");
            }
        }
    }
}
