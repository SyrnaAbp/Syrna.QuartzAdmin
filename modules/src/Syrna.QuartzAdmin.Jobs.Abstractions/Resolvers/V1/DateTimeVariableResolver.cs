...
namespace Syrna.QuartzAdmin.Jobs.Abstractions.Resolvers.V1
{
    ...
    {
        const string DatetimeRegex = $"\\{VariableNameContants.DateTime}\\s(rfc1123|iso8601|\'.+\'|\\\".+\\\")(?:\\s(\\-?\\d+)\\s(y|M|d|h|m|s|ms))?";
        ...
        public string Resolve(string varBlock)
        {
            ...
            if (!string.IsNullOrEmpty(rawOffset))
            {
                ...
                {
                    throw new FormatException($"Invalid {GetVariableName()} pattern. Offset '{rawOffset}' should be numeric value.");
                }
                ...
                switch (offsetOption)
                {
                    ...
                    case "ms":
                        ...
                        break;
                    default:
                        throw new FormatException($"Invalid {GetVariableName()} pattern. Need to provide valid offset option.");
                }
            }
