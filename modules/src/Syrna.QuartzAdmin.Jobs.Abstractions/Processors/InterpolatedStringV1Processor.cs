...
namespace Syrna.QuartzAdmin.Jobs.Abstractions.Processors
{
    public class InterpolatedStringV1Processor
    {
        ...
        public string Process(DataMapValue interpolatedString)
        {
            ...
            _logger?.LogDebug("Processing interpolated string [{string}]", interpolatedString.Value);
            foreach (Match match in Regex.Matches(interpolatedString.Value, VariableRegex, RegexOptions.None))
            {
                _logger?.LogDebug("Matched '{matchValue}' on index {index} length {length}.",
                    match.Value, match.Index, match.Length);
                strBldr.Append(interpolatedString.Value.Substring(lastIndex, match.Index - lastIndex));
